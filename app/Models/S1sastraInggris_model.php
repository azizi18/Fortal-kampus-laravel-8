...
class S1sastraInggris_model extends Model
{
    protected $table         = "s1sastra_inggris";
    ...
    {
        $query = DB::table('s1sastra_inggris')
        ->join('users', 'users.id_user', '=', 's1sastra_inggris.id_user', 'LEFT')
            ->select('s1sastra_inggris.*', 's1sastra_inggris.nidn', 's1sastra_inggris.nik_nip', 's1sastra_inggris.nama', 's1sastra_inggris.email')
            ->orderBy('s1sastra_inggris.id_sas', 'DESC')
            ->get();
        return $query;
    }
    // listing
    ...
    public function detail($id_sas)
    {
        $query = DB::table('s1sastra_inggris')
            ->join('users', 'users.id_user', '=', 's1sastra_inggris.id_user', 'LEFT')
            ->select('s1sastra_inggris.*', 's1sastra_inggris.nidn', 's1sastra_inggris.nik_nip', 's1sastra_inggris.nama', 's1sastra_inggris.email')
            ->where('s1sastra_inggris.id_sas', $id_sas)
            ->orderBy('id_sas', 'DESC')
            ->first();
        return $query;
    }
    ...
    public function cari($keywords)
    {
        $query = DB::table('s1sastra_inggris')
             ->join('users', 'users.id_user', '=', 's1sastra_inggris.id_user', 'LEFT')
             ->select('s1sastra_inggris.*', 's1sastra_inggris.nidn', 's1sastra_inggris.nik_nip', 's1sastra_inggris.nama', 's1sastra_inggris.email')
            ->where('s1sastra_inggris.nama', 'LIKE', "%{$keywords}%")
            ->orderBy('id_sas', 'DESC')
            ->get();
        return $query;
    }
